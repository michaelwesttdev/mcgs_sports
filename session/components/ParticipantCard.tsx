...
export default function ParticipantCard({ participant, onUpdate,createHouse,fetchHouses,houses, onDelete,house, style }: ParticipantCardProps) {
    const navigate = useNavigate();
    const {sessionId} = useSessionState();
    return (
        <TableRow style={style} key={participant.id} onClick={()=>navigate(`/sessions/${sessionId}/participant/${participant.id}`)}>
