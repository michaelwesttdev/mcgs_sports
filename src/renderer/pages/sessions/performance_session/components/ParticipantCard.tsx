...
export default function ParticipantCard({ searchString,participant, onUpdate,createHouse,fetchHouses,houses, onDelete,house }: ParticipantCardProps) {
    const navigate = useNavigate();
    const {sessionId} = useSessionState();
    return (
        <TableRow key={participant.id} onClick={()=>navigate(`/sessions/${sessionId}/participant/${participant.id}?searchP=${searchString}`)}>
